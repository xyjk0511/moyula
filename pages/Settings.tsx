...
const Settings = () => {
  const { salaryPerHour, setSalaryPerHour, isDarkMode, toggleDarkMode } = useMoyuu();
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto no-scrollbar pt-6 px-6 gap-6 pb-24">
       <header className="flex items-center justify-between">
           ...
           <button className="size-10 rounded-full bg-creamy-card flex items-center justify-center shadow-card border border-gray-100 dark:border-white/10">
       ...
       <div className="bg-creamy-card p-6 rounded-[1.5rem] shadow-card border border-white dark:border-white/10 flex items-center gap-4">
           <div className="relative size-16 rounded-full overflow-hidden border-4 border-primary/10">
               <img src="https://picsum.photos/100/100?random=2" className="w-full h-full object-cover" alt="User" />
           </div>
           <div className="flex-1">
               <h2 className="text-lg font-bold text-text-main">摸鱼专员 #007</h2>
               <p className="text-sm text-text-sub">已坚持摸鱼 128 天</p>
           </div>
       ...
       <button className="w-full mt-2 py-4 text-red-400 font-bold text-sm bg-red-50/50 dark:bg-red-900/20 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-2xl transition-colors">
            退出登录
       </button>
    </div>
  );
